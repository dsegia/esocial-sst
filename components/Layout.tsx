...
const MENU = [
  ...
  { href: '/funcionarios', label: 'Funcionarios',   icon: 'M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 7a4 4 0 100 8 4 4 0 000-8z' },
...
export default function Layout({ children, pagina }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f4f6f9', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      ...
      <div style={{ width: 220, flexShrink: 0, background: '#fff', borderRight: '0.5px solid #e5e7eb', display: 'flex', flexDirection: 'column', padding: '1.25rem 0' }}>
        ...
        <nav style={{ flex: 1, padding: '0 .75rem' }}>
          {MENU.map(function(item) {
            var ativo = pagina === item.href.replace('/', '')
            return (
              <a key={item.href} href={item.href} style={{
                ...
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none"
                  stroke={ativo ? '#185FA5' : '#9ca3af'} strokeWidth="2">
                  <path d={item.icon}/>
                </svg>
                {item.label}
              </a>
